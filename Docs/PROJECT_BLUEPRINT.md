...
12. **Never wrap platform semantics** — Chỉ tokenize/abstract những **năng lực nền tảng KHÔNG cung cấp**; **không bao giờ** bọc lại thứ nền tảng đã biểu đạt bằng ngữ nghĩa. Bằng chứng chuẩn (M9-3): SwiftUI đã có hệ typography ngữ nghĩa đầy đủ (`.headline/.subheadline/.caption`, Dynamic-Type) → `Typography.swift` chỉ đổi tên 1:1 = ngôn ngữ thứ hai vô ích → **xoá**. Ngược lại `OsirisColor/Spacing/Radius` hợp lệ vì nền tảng KHÔNG có palette dark-only thương hiệu / thang spacing / thang radius. Kiểm tra trước mọi token mới: *"nền tảng đã biểu đạt cái này bằng ngữ nghĩa chưa?"* — nếu rồi, dừng. Đây là "Simplicity First" + "Reuse Before Create" áp cho tầng thiết kế.
...
| AD-26 | Event Bus xếp ngang hàng Kernel/Gateway trong Core | Thổi phồng cognitive load; nó là pub/sub mỏng trong app đơn tiến trình | ~~**Hạ Event Bus xuống Infrastructure**~~ → **superseded bởi AD-46** (xóa tại deadline M4 — 0 consumer động) |
...
| AD-47 | M6-0 mở milestone Automation + Risky Actions — bắt buộc Architecture Review trước code (4 quyết định, đều bằng chứng) | **(1) XÓA ApprovalGate** (+ RiskyAction/ApprovalDecision/RequireUserApprovalGate): grep chứng minh `evaluate(_:)` chưa từng được gọi và `RiskyAction` chưa từng construct qua 6 milestone; automation v1 chạy skill sinh deliverable (local, reversible) = KHÔNG phải risky action. Xóa (146/146 pass, 0 test logic sửa — bằng chứng không consumer). Shape cũ (publish/delete/largeSpend…) là phỏng đoán M0 — giữ sẽ neo thiết kế tương lai vào phỏng đoán. **Tái sinh:** cùng sự kiện với risky action THẬT đầu tiên (external/irreversible) — cũng chính là lúc mở tool-channel (AD-45); một quyết định thống nhất, không ba mảnh chết rải rác. **(2) Tool-channel: KHÔNG mở** — không module nào ở M6-0 cần (AD-45 giữ nguyên). **(3) EventBus: KHÔNG tái sinh** — automation v1 không tạo audience động; fan-out closure vẫn đủ (AD-46 giữ nguyên). **(4) Automation sống ở đâu:** = DATA (rule mô tả goal + trigger) + tái dùng vòng đời Kernel hiện có; KHÔNG Engine/Runtime/Scheduler. Trigger theo lịch = iOS background (PENDING thiết bị). Build ở M6-1 với schema tối thiểu (AD-28) khi trigger model rõ — M6-0 chỉ chốt thiết kế + dọn scaffolding chết |
| AD-46 | M4-4: deadline EventBus đã điểm (AD-39 hẹn: "tái đánh giá M4, xóa nếu modules không dùng") — 4 mảng nghiệp vụ module liên tiếp không cần events; audience của bus là 1 producer (Kernel qua closure AD-33) + 2 consumer TĨNH (chat relay, dashboard) | **XÓA EventBus** — thực thi đúng kế hoạch đã ghi: actor 31 dòng đứng giữa 1 producer và 2 closure tĩnh là indirection thuần overhead; thay bằng fan-out trực tiếp trong closure `publish` tại composition root (2 dòng — seam AD-33 không đổi, Kernel không biết gì thay đổi); test EventBus xóa theo component; thêm "EventBus" vào danh sách cấm-tái-tạo (arch rule). **Điều kiện tái sinh:** audience ĐỘNG thật (subscriber đăng ký/hủy lúc runtime — vd module states M5+, automation M6) — khi đó bus được earn lại bằng bằng chứng, qua đúng seam này. AD-26 → Superseded |
| AD-45 | M4-3: Channel Analysis là capability đầu tiên cần dữ liệu thật từ ngoài — có mở kênh đóng góp `tools` cho ModuleManifest không? (câu hỏi đã hẹn từ M4-0) | **KHÔNG mở — chưa có bằng chứng buộc, và có một lý do cấu trúc:** `ModuleManifest` là `Codable` (thuần data, AD-44); `Tool` là protocol mang HÀNH VI (không Codable) — nhét `[any Tool]` vào manifest đổi BẢN CHẤT contract từ data sang code, không phải chỉ thêm field. Channel Analysis v1 = skill phân tích **dữ liệu user cung cấp** (dán từ YouTube Studio vào goal): 0 tool, 0 OAuth, 0 network, 0 state trong module; insight từ text là việc AI thật sự — không tool deterministic nào làm được ("AI Is The Last Tool" giữ nguyên); dữ liệu dán luôn tươi đúng mức user muốn (cùng logic AD-37). **Điều kiện kích hoạt mở kênh tools (M6 Automation):** user quyết OAuth/network/privacy + MCP flag bật; khi đó phương án ưu tiên là tách phần code khỏi manifest (tool inject ở composition root theo module, hoặc declaration thứ hai không-Codable) — quyết với bằng chứng thật, KHÔNG thiết kế trước hôm nay |
| AD-44 | M4-0: cần định nghĩa "module là gì" trước module đầu tiên; mọi hướng Plugin Engine / Module Manager / Extension Framework đều là máy móc chưa có bằng chứng | **Module Contract v1: module LÀ manifest của nó — thuần data.** `ModuleManifest` (Core/Modules/Contracts): id/version/purpose/skills — registry thứ hai được phép theo AD-17 và nó là DESCRIPTOR, không phải engine. Skills là kênh đóng góp duy nhất v1 (skill đã chở đủ: template/keywords/tier/composition — AD-28); kênh mới (tools, event consumers) chỉ thêm khi module thật không ship được nếu thiếu. **Ranh giới 3 lớp cưỡng chế:** (1) compiler — target `OsirisModules` chỉ depend OsirisCore, Core không depend Modules (Core không thể biết module); (2) arch rule — Modules chỉ import OsirisCore, chỉ dùng data types (cấm Kernel/Gateway/Store/Execution/I-O); (3) arch rule — Core/Application/Infrastructure/Presentation cấm nhắc tên module cụ thể; composition root là nơi DUY NHẤT biết danh sách module cài đặt. Namespace skill theo module id là precondition structural (không collision với core.* hay module khác). Module skills đi qua đúng registry + matcher + lifecycle hiện có — Kernel không phân biệt built-in hay module |
| AD-43 | M3-3: deliverable cần Executive Summary + next steps nhất quán, nhưng mọi hướng "Summary Engine / Report Builder / Formatter Engine" đều là component mới chỉ phục vụ một format | **Executive Summary không phải component — là cấu trúc output KHAI BÁO.** Scaffold là DATA trong `Config/deliverable-scaffold.md` (test cưỡng chế ≤80 token — không phải preamble thứ hai), composition root inject vào Execution Engine như chuỗi khai báo; Execution NỐI MÁY MÓC vào task (sau template + previous-step), không quyết định gì. Áp dụng đúng nơi sinh deliverable: đường `.ai` và BƯỚC CUỐI composition (bước giữa là nguyên liệu, không scaffold); `.reuse` trả nguyên văn (không re-format nội dung cũ); `.tool` không AI nên không scaffold. Default `nil` = prompt byte-identical trước M3-3 (zero regression by construction). Sửa format mọi deliverable = sửa MỘT file Config, không chạm code. Không Summary/Report/Formatter Engine — 1 optional String + 1 phép nối |
| AD-42 | M3-2: `plan.preferredTier` đang hardcode `.light` và Confidence Medium chưa có consumer; cám dỗ tự nhiên là "planner thông minh" (scoring engine / học từ lịch sử) — đi ngược nguyên tắc planner giải thích được | **Smart Planning v1 hoàn toàn deterministic.** `ComplexityEstimate` (pure, `Core/Kernel/Decision/`) ước lượng `simple/standard/complex` chỉ từ dữ liệu có sẵn trong goal (số từ, tín hiệu khối lượng khai báo "detailed/toàn diện/đầy đủ…", số mệnh đề qua dấu câu + liên từ) — cùng input cùng output, 0 AI, 0 token, 0 lịch sử. **Tier được earn, không hardcode:** skill khai báo tier > estimate (skill hiểu việc của nó nhất); Gateway chỉ tiêu thụ tier, không bao giờ quyết (routing kích hoạt khi catalog ≥2 model thật). **Confidence Medium có consumer đầu tiên:** tín hiệu mơ hồ chặt ("something/somehow/gì đó/sao cũng được…") → vẫn thực thi nhưng assumption được ghi **qua đúng WriteGate M3-1** (justification `reusableLater`, WorkingContext TTL tự dọn) — KHÔNG tồn tại đường ghi thứ hai; gate `.disabled` chặn cả assumption (có test). Confidence phản ánh độ chắc của quyết định, không phải "trí thông minh" — không bao giờ do AI điều chỉnh. **Ranh giới mở rộng có chủ đích:** extension-by-DATA mở (thêm skill/tool/policy/provider không sửa Kernel); extension-by-ALGORITHM đóng (heuristic constants, matcher, resource order là nội bộ Kernel) cho đến khi có bằng chứng cần seam |
| AD-41 | M3-1: hệ thống cần bắt đầu tự ghi nhớ, nhưng learning không kiểm soát là rủi ro lớn nhất của Intelligence Layer (AD-20 đã cảnh báo từ v1.1) | **Reflection v1 hoàn toàn deterministic** — 0 AI, 0 token: chỉ nhìn dữ liệu đã có (strategy/goal/deliverable), tiêu chí chặt (chỉ `.ai/.composition` + goal ≥4 từ + có deliverable), tối đa MỘT candidate. **Chuỗi trách nhiệm bằng type:** Reflection sinh `MemoryCandidate` (không biết Store, không persist) → `WriteGate` là nơi DUY NHẤT quyết định ghi/không/ghi-đâu (policy từ policies.json, unknown justification = fail fast; đích v1 = WorkingContext có TTL tự dọn; Knowledge đóng cho đến khi có consumer) → Store persist. Không đường bypass — arch rule mới: memory record chỉ được construct trong Core/Store. Save best-effort (memory phụ trợ không được làm fail goal). **AI-reflection bị cấm** cho đến khi trả lời được 4 câu: deterministic vì sao không đủ / AI thêm giá trị gì / bằng chứng thật / token có đáng. AD-20: AWAITING → **PROVEN** |
| AD-40 | Advanced Mode toggle (M2-5) cần persist, nhưng `features.json` là bundle read-only trên iOS, còn nhét UI-pref vào Store là nhiễm khái niệm UI vào platform data (WorkingContext còn sai cả TTL semantics) | **Ranh giới hai loại persist:** *UI preferences* (do Presentation sở hữu, không bao giờ là input cho Kernel/Gateway/Store) sống ở UserDefaults/@AppStorage — chỉ được dùng trong App/ + Presentation/; *platform data* vẫn chỉ qua Store (AD-32 nguyên vẹn). Cưỡng chế bằng arch rule mới: Core/Application/Infrastructure cấm UserDefaults/AppStorage. Advanced Mode nhờ vậy nằm trọn trong Presentation — không cần port |
| AD-39 | Dashboard (M2-4) cần đọc token usage nhưng metrics chỉ ra log; đồng thời không được biến Gateway thành telemetry system | **Một seam quan sát duy nhất:** `DefaultAIGateway.onMetrics` — callback optional (default nil, zero regression) phát `AIRequestMetrics` mỗi request; Gateway không aggregate, không persist — consumer quyết định. `DashboardModel` (Application) giữ số liệu **session-only** (lịch sử dài hạn = câu hỏi M7, quyết bằng dữ liệu thật). **EventBus có consumer production đầu tiên** (chat relay + dashboard activity qua 2 subscription); đánh giá trung thực: giá trị hôm nay ≈ closure fan-out với 2 consumer tĩnh — giá trị thật ở consumer động (Modules M4); tái đánh giá tại M4, xóa nếu modules không dùng |
