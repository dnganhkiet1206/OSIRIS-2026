...
| Hạng mục | Giá trị |
|---|---|
| Giai đoạn | **RELEASE PHASE — v0.1.0-alpha PREPARED + RC-VALIDATED (§4ac/§4ad); READY TO TAG.** R2 = YES approve publish (0 blocker; iPhone validated, iPad/macOS universal-build chưa verify = scope decision). M9 CLOSED. Tag `v0.1.0-alpha` + merge PR chờ release owner. · lịch sử M9: **M9 — Product Experience & UI/UX** (M0→M8 nghiệm thu; tag local chờ push). M9-0 Review ✅ (§4t) · **M9-1 Design System Foundation ✅ (§4v)** · **M9-2 Spacing System ✅ (§4w)** · **M9-3 Clarity & Typography ✅ (§4x, USER nghiệm thu — Typography.swift XOÁ sau review, CI bc0e5e0 xanh)** · **M9-4 Palette Rollout ✅ (§4y, CI xanh)** · **M9-5 IA & UX Review ✅ (§4z, CI xanh — fix: Search no-results)** · **M9-6 Product Experience Review ✅ (§4aa)** · **M9-RC1 Release Blocker Cleanup ✅ (§4ab, CI xanh)** → **READY FOR RELEASE CANDIDATE (Alpha)** — không còn release blocker đã biết; non-blocking mục (M7 key / a11y runtime / BGTask+MCP / M9 surface / 2 UX-arch) verify on-device trước wider release. **Design System KHÓA.** KHÔNG mở rộng AI/module/tool — chỉ biến prototype → sản phẩm production |
| Task hiện tại | **R2 — Alpha RC Validation ✅ (§4ad, audit-only) — YES approve publish v0.1.0-alpha.** 0 blocker (aiDryRun:false=online thật; 0 Debug/Release divergence; precondition = wiring guard; iPhone §4i). iPad/macOS universal nhưng chưa device-verify (scope decision, KHÔNG blocker). 0 code change. Trước đó: **R1** (2d64448, version 0.1.0/1 + release notes) · **R0** (READY FOR ALPHA). Set `MARKETING_VERSION 0.1.0` + `CURRENT_PROJECT_VERSION 1` (`project.yml`; sửa "dev" trong app), CHANGELOG → `[0.1.0-alpha]` + release notes, README mục Release. 0 code/arch/feature. **Decision: YES — READY TO TAG v0.1.0-alpha** (tag chưa tạo — release owner: `git tag v0.1.0-alpha && git push`). Non-blocking: app icon / a11y runtime / provider-side / scheduled+MCP. Trước đó: **M9-RC1** (32681d7, breadth whole-word regression) + **R0 Release Readiness Audit = READY FOR ALPHA**. Bug latent ComplexityEstimate substring ("carefully"⊃"full") ĐÃ fix (6f43ac0, whole-word match, 0 NLP/regex/architecture, heuristic nguyên) + **strengthen regression** (9 assertion word-boundary). **Re-eval: KHÔNG còn release blocker đã biết cho Alpha → READY FOR RC.** Cả 2 Devin finding (provider/model mismatch + breadth substring) ĐÓNG. Trước đó: M9-6 Product Experience (7b46635, Deliverable Done button). Review journey first-time-user; **phần lớn tốt** (empty state chào, feedback tức thì, spinner activity, error message friendly+actionable) → **1 fix demonstrable common-path**: sheet Deliverable thiếu dismiss control → **toolbar "Done" nền tảng** (0 abstraction, 0 logic, tuân "platform components preferred"). Ghi KHÔNG đổi (architecture/debatable): retry-sau-failure, clarification-threading, return-key-send, Dashboard Loading text. Trước: M9-5 Search no-results (`ContentUnavailableView`). Review UX toàn app first-time-user; app phần lớn tốt → **1 fix demonstrable**: `SearchResultsView` thiếu no-results state (vùng trống dưới field trông như hỏng) → **`ContentUnavailableView` nền tảng** (iOS 17, 0 abstraction, tuân #12), `@State` chặn hiện sớm; 0 màu/spacing/typography/logic. Ghi (KHÔNG đổi, cần kiến trúc): Chat/Projects nav ambiguity, restart-đổi-key friction. "Không đổi" chỗ khác = hợp lệ (không ép việc). Trước đó: **M9-4 Palette Rollout ✅** (b54931f success: `.secondary`→`OsirisColor.textSecondary` 11 site AA+; GIỮ Apple tertiary/destructive/quaternary). Áp `OsirisColor` lên view còn dùng màu semantic Apple. **Architecture Review (contrast WCAG):** `.secondary`→`OsirisColor.textSecondary` (#A8A8A8 AA+ mọi surface: base 8.4 / card #181818 7.5 / list #1C1C1E 7.2) qua **11 site** — token nay được dùng. **GIỮ Apple** (objectively better): `.tertiary` (flat #7C7C7C **rớt AA** card 4.25 / list 4.08), `role:.destructive` (an toàn), badge `.quaternary` (subtle fill). **CHƯA áp:** `textTertiary` (flat không surface-safe), `textPrimary`/`background`/`elevated` (surface-migration hoãn). Màu-only, 0 a11y/DynamicType/dark impact. Tuân #11+#12. · kế tiếp: **M9-5 chờ USER** — KHÔNG tự mở |
...
## 4ad. R2 Closeout — Alpha Release Candidate Validation (2026-07-06, audit-only)

**Task (Release phase):** validation cuối RC v0.1.0-alpha. Chỉ verify, KHÔNG feature/redesign/refactor. Fix CHỈ true blocker.

**Bằng chứng mới (ngoài R0/R1):**
- **`Config/features.json` `aiDryRun: false`** → online mode gọi AI THẬT (không dry-run) ✅; `mcpRemoteTools:false`, `advancedMode:false` (default đúng).
- **0 `#if DEBUG`** → không có phân kỳ Debug/Release; **Debug CI build = đại diện hành vi Release** ✅ (SPM release 0/0).
- **Precondition** (`SkillDefinition:65`, `ModuleManifest:39`, `ChatService:46`) = guard construction/wiring trên static data đã test — KHÔNG phải crash workflow người dùng.
- **`routing.json`**: default `claude-haiku-4-5-20251001` (thật) / offline `placeholder-local`; GatewayConfiguration validate (ShippedConfig test).
- **Device family:** `project.yml` chỉ `platform: iOS`, KHÔNG `TARGETED_DEVICE_FAMILY` → universal (iPhone+iPad, +Mac via "Designed for iPad"). **CHỈ iPhone validated (§4i).**

**Risk matrix:** iPad/macOS chưa device-verify (**Medium** — universal build + NavigationSplitView adaptive, chưa runtime proof); Archive/Release config chưa CI-test (**Low** — không divergence); app icon (**Low**); signing chưa trong repo (**Low**). **KHÔNG Critical/High, KHÔNG blocker.**

**Decision: YES — approve publishing v0.1.0-alpha.** Mọi blocker category sạch (correctness/crash/data-loss/security/arch/regression/workflow); online thật + offline an toàn; iPhone end-to-end (§4i); version set. Non-PASS = verification gap / branding, không phải defect. **Quyết định scope (release owner, KHÔNG blocker):** ship iPhone-only (validated) hay verify iPad/macOS trước — `TARGETED_DEVICE_FAMILY` là quyết định của owner, KHÔNG tự đổi.

**KHÔNG code change (0 blocker).** KHÔNG tự tag, KHÔNG tự merge PR — hành động release owner.

## 4ac. R1 Closeout — Versioning & Alpha Release Preparation (2026-07-06, CI xanh)

**Task (Release phase, KHÔNG feature/redesign/architecture):** chuẩn bị tag **v0.1.0-alpha**. R0 đã kết luận READY FOR ALPHA; R1 set version + release notes.

**Audit versioning (bằng chứng):** `project.yml` **CHƯA set** `MARKETING_VERSION`/`CURRENT_PROJECT_VERSION` (Info.plist auto qua `GENERATE_INFOPLIST_FILE`) → app version undefined, `AdvancedView:51` fallback "dev". Bundle id `com.osiris.Osiris` ✅; display name OSIRIS ✅; launch screen auto ✅. **App icon ABSENT** (0 `.xcassets`) — WARNING, Alpha-acceptable, Design System KHÓA nên KHÔNG thêm. CI build **Debug** (không `-configuration Release`) — WARNING (cùng code path). git clean; `git tag` rỗng ở clone này (tag milestone local-in-user-env).

**Thay đổi nhỏ nhất (release surface only):**
- `project.yml`: `MARKETING_VERSION: "0.1.0"` + `CURRENT_PROJECT_VERSION: "1"` (CFBundleShortVersionString numeric; "-alpha" ở tag + notes). Sửa luôn "dev" hiển thị trong app.
- `CHANGELOG.md`: `[Unreleased — M9]` → **`[0.1.0-alpha] — 2026-07-06`** + release-notes summary (là gì / highlights / quality gate / known non-blocking limitations).
- `README.md`: mục Release ngắn (version, notes, tag/archive steps).
- **0 code/architecture/feature change.**

**Risk matrix:** app icon (Low), Release-config chưa CI-verify (Low), signing chưa cấu hình repo (Low). KHÔNG Critical/High. KHÔNG correctness/crash/security/arch/regression blocker.

**Self/Arch/Quality:** yml/README/CHANGELOG không phải Swift → 0 arch impact; Linux SwiftPM không đụng; macOS CI `xcodegen generate`+`xcodebuild` verify project build với version fields. Verify = CI **2d64448 success** (Linux + macOS). $0.00.

**Decision: YES — READY TO TAG v0.1.0-alpha.** Version + notes đã set, CI xanh. **Tag chưa tạo** (hành động của release owner): `git tag v0.1.0-alpha <commit> && git push origin v0.1.0-alpha` — quyết định tag trên branch hay merge PR #1 vào main trước (convention: merge rồi tag merge commit). Non-blocking (icon / a11y runtime / provider-side / scheduled+MCP) verify sau/on-device.

## 4ab. M9-RC1 Closeout — Release Blocker Cleanup (2026-07-06, CI xanh)

**Task:** loại bỏ release blocker latent (ComplexityEstimate substring false-positive) trước Alpha. Fix nhỏ nhất, KHÔNG redesign/NLP/regex-nặng/architecture. Regression proof.

**Bối cảnh:** Devin (PR #1) báo bug latent hợp lệ — `breadthSignals` substring match: "carefully"⊃"full" → misclassify `.complex`. 0 impact runtime hôm nay (single-model → `preferredTier` no-op); thành bug thật khi bật multi-model routing (M7).

**Fix (ĐÃ làm 6f43ac0, CI xanh — trước khi milestone mở):** single-word signal match **whole-word** (token split trên non-letter, `Set.contains`); multi-word/hyphen ("in-depth"/"chi tiết"/"toàn diện"/"đầy đủ"/"chuyên sâu") giữ substring (an toàn — cụm không nằm trong từ khác). Deterministic + pure (AD-42 nguyên); heuristic KHÔNG đổi (wordCount/breadthSignal/clauseCount); 0 NLP/regex-nặng; smallest correct change.

**M9-RC1 (32681d7):** **strengthen regression** (test-only, USER yêu cầu edge case) — 9 assertion: "full" trong carefully/fully, "complete" trong autocomplete/completely/incomplete → `.simple` (đều `.complex` dưới code cũ); "full"/"complete"/"comprehensive"/"chi tiết" whole-word → `.complex`. Existing 13 M3-2 assertion giữ nguyên.

**Self/Arch/Quality:** Kernel purity giữ (pure String, 0 I/O); test chỉ THÊM; 0 architecture/production change ở M9-RC1 (fix ở 6f43ac0). Verify = CI **32681d7 success** (Linux suite + macOS). $0.00.

**Release-blocker re-evaluation (Alpha):** 2 finding correctness của Devin đều ĐÓNG (provider/model mismatch §CHANGELOG + breadth substring). Không còn bug correctness/stability đã biết. **Non-blocking đang mở (đã track, KHÔNG chặn Alpha):** (a) M7 provider-side cost/latency optimization — cần standing key; (b) a11y runtime pass (VoiceOver/Dynamic Type/contrast) — cần device (source-level đã sạch M8-6); (c) scheduled BGTask + MCP — feature hoãn có điều kiện; (d) M9 surface migration + textTertiary rework — UI polish; (e) 2 UX observation (Chat/Projects nav, key-restart) — cần architecture, USER quyết. → **KHÔNG còn release blocker đã biết cho Alpha; codebase READY FOR RELEASE CANDIDATE** (với các mục non-blocking trên verify on-device trước wider release).

## 4aa. M9-6 Closeout — Product Experience Review (2026-07-06, CI xanh)

**Task:** review OSIRIS như sản phẩm first-time user thật dùng. KHÔNG polish/design-system/redesign/architecture. Tìm **friction trong user journey thật**. "Không đổi" = hợp lệ.

**Architecture Review (audit journey, bằng chứng code):** journey **phần lớn tốt** — first-launch empty state chào ("What do you want to accomplish?"), gửi goal → bubble user tức thì + spinner activity live, kết quả → OSIRIS bubble. **Error messaging THẬT tốt** (`ChatService.translate`): friendly + actionable đúng UI contract ("try rephrasing your goal", "check your connection and try again", "stopped before spending anything"). Waiting text rõ ("Understanding…/Planning…/Working on it…/Saving progress…"). → đa số hạng mục KHÔNG cần đổi.

**Lỗ hổng DEMONSTRABLE duy nhất (sửa, common path, platform-native):** sheet đọc Deliverable (`ChatView`, mở từ "Recent deliverables" + Search) có title nhưng **KHÔNG có dismiss control** → chỉ đóng bằng swipe-down. Vòng mở/đóng deliverable là core loop sớm; first-time/ít-quen-iOS lúng túng "làm sao quay lại". Sửa = **toolbar "Done" button nền tảng** (`.confirmationAction`) clear `openedDeliverable` (binding đã có) — 0 abstraction, 0 logic/VM change, tuân "platform components preferred".

**Demonstrable nhưng NGOÀI phạm vi (ghi, KHÔNG đổi — architecture/debatable/polish):**
- **Không retry sau failure:** goal xoá khi gửi → retry = gõ lại. Thật nhưng failure hiếm ở path first-time (offline→placeholder completions, không phải failure); retry cần VM state mới.
- **Clarification answer bị xử như goal mới** (không thread context) → sửa = feature conversation-context = architecture.
- **Return key không gửi** ở composer vertical (button chạy; không có fix platform sạch).
- **Dashboard "Loading…"** text trần (polish marginal, resolve tức thì).

**Self/Arch/Quality Review:** 18 arch rule giữ; 0 type/manager/engine/framework mới (chỉ platform ToolbarItem+Button); a11y/Dynamic Type/dark do button chuẩn lo; 0 màu/spacing/typography/logic; 0 SPM target → suite Linux không đổi.

**Verify = CI (xanh):** run **7b46635 = success** (Linux + macOS compile). AI spend **$0.00**.

**Kết luận: M9-6 code-complete + CI xanh.** Journey review đúng kỷ luật — 1 friction common-path sửa bằng platform convention, phần còn lại đã tốt hoặc ngoài phạm vi (ghi rõ). M9-5+M9-6 = 2 review liên tiếp, mỗi cái đúng 1 fix platform-native nhỏ, phần lớn app xác nhận đã tốt.
...
- `App/OsirisApp.swift`: `.preferredColorScheme(.dark)` + `.tint(accent)` — dark-only được BẢO ĐẢM ở root.
- `ProjectResumeView`: `.osirisCard()` cho container + inset deliverable rows (dùng `elevated`) — **card-surface**; text giữ **semantic** (`.secondary`/`.tertiary`).
...
- **Palette = catalog khai báo** (single source of truth cho V1, gồm quyết định nudge AA): M9-1 ÁP `card`/`elevated`/`border`/`accent`/`Radius`; `background` + `textPrimary/Secondary/Tertiary` **khai báo, áp ở milestone rollout sau** (không phải dead code — là data catalog). Mọi accessibilityLabel/semantic font/SecureField giữ nguyên (M8-6 sạch).

**Design Review độc lập + tinh chỉnh (USER duyệt Rec-2, 2026-07-06):** review 4 mặt trên chính quyết định M9-1 → **2 sửa nhỏ trước merge** (không phải feature mới): **(a)** rollback text-color migration ở `ProjectResumeView` về semantic — bản đầu đã áp `OsirisColor.textX` chỉ 1 view = **giới thiệu ngôn ngữ thị giác thứ hai** (xám `#A8A8A8` ≠ system `.secondary` của các view khác) → vi phạm chính "Consistency #1" + vượt Q2; text-color rollout để milestone riêng (app-wide). **(b)** gỡ `.background(OsirisColor.background)` ở root — `NavigationSplitView` còn sở hữu nhiều system surface đè lên → nền chỉ là **partial**; surface rollout hoàn chỉnh để bước sau. Giữ: foundation (OsirisColor/Radius/.osirisCard()), card-surface rollout (trọn tập proven), dark-only lock, accent tint, `#7C7C7C`. → nguyên tắc **Incremental Product Evolution** (BLUEPRINT §2/#11) sinh ra từ review này.

**Dark-Only = PRODUCT DECISION có chủ đích (không phải tai nạn triển khai):** OSIRIS khoá dark (`.preferredColorScheme(.dark)`) theo Design Language V1 của USER. Palette hex cố định (không adaptive) → **bắt buộc** khoá dark để không vỡ trên Light-OS. **Đường di trú Auto/Light tương lai (KHÔNG làm bây giờ):** (1) `OsirisColor` → **adaptive token** (asset colorset hoặc `Color(light:dark:)`); (2) đổi khoá cứng → **user preference** qua **`@AppStorage`** (đúng AD-40: UI-pref ở App/Presentation); (3) bổ sung light palette. Dòng khoá 1-dòng, đảo ngược tầm thường — không phải one-way door kiến trúc.
